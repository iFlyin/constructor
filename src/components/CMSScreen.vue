...
export default class CMSScreen extends Vue {
    ...
    private get minWidth(): number {
       ...
       for (const child of this.childsList) {
          const childMaxX = child.params.X + 170;
    ...
    private get minHeight(): number {
        ...
        if (this.childsList.length > 0) {
            for (const child of this.childsList) {
                const el = this.$el.querySelector(`div[data-id="${child.props.id}"]`);
    ...
    private drop(payload: any): void {
        ...
        if (posX < 0) { posX = 0 }
        if (rightX > this.width) { posX = this.width - item.params.width - 10; }
        let posY = e.offsetY / this.zoom - centerY;
        const bottomY = posY + item.params.height;
        if (posY < 0) { posY = 0 }
        if (bottomY > this.height) { posY = this.height - item.params.height - 20; }
        item.params.X = posX;
        item.params.Y = posY;
